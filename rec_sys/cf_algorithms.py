...
def rate_all_items(orig_utility_matrix, user_index, neighborhood_size):
    ...
    def rate_one_item(item_index):
        ...
        if best_among_who_rated.size > 0:
            ...
            #take bias of all best_among_who_rated users into account and substract it from their ratings 
            unbiased_ratings = orig_utility_matrix[item_index, best_among_who_rated] - np.nanmean(orig_utility_matrix[:,best_among_who_rated],axis=0)
            rating_of_item = (similarities[best_among_who_rated] @ unbiased_ratings) /np.abs(similarities[best_among_who_rated]).sum()
            #add the bias mean for current user to the rating of item
            rating_of_item += np.nanmean(orig_utility_matrix[:,user_index])
        else:
